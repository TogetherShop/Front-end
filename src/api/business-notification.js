...
export const getBusinessNotifications = async () => {
  try {
    ...
    console.log('getBusinessNotifications 요청 전 토큰 확인:', {
      hasToken: !!token,
      tokenLength: token?.length,
      userType: localStorage.getItem('user_type'),
    })

    const response = await api.get('/api/notifications/business', {
      userType: 'business',
    })
    console.log('사업자 알림 목록 조회:', response.data)
...
export const markNotificationAsRead = async (notificationId) => {
  try {
    console.log('알림 클릭 처리 요청:', notificationId)
    
    const response = await api.post(`/api/notifications/business/${notificationId}/read`, {}, {
      userType: 'business',
    })
    console.log('알림 클릭 처리 성공:', response.data)
    return response.data
  } catch (error) {
...
export const markAllNotificationsAsRead = async () => {
  try {
    ...
    const response = await api.put('/api/notification/business/read-all', {}, {
      userType: 'business',
    })
