 ...
 * @param {string} params.storeGroup - 매장 그룹 필터
 ...
 * @param {string} data.storeId - 사용 매장 ID
 ...
 * @param {string} params.type - 히스토리 타입 (claim, use, expire)
 * @param {string} params.startDate - 시작 날짜
 * @param {string} params.endDate - 종료 날짜
 * @param {number} params.page - 페이지 번호
 * @param {number} params.limit - 페이지당 아이템 수
 * @returns {Promise<Object>} 쿠폰 히스토리 데이터
 */
export const getCouponHistory = async (params = {}) => {
  try {
    ...
    console.error('쿠폰 히스토리 조회 실패:', error)
    throw error
  }
}
